...
class FiBiNET(BaseModel):
    ...
    def __init__(self, linear_feature_columns, dnn_feature_columns, bilinear_type='interaction',
        ...
        self.dnn = DNN(self.compute_input_dim(dnn_feature_columns) + self.bilinear_compute_input_dim(dnn_feature_columns),
    ...
    def bilinear_compute_input_dim(self, feature_columns, include_sparse=True, include_dense=True):
    ...
    def forward(self, X):
        ...
        sparse_embedding_list, dense_value_list = self.input_from_feature_columns(X, self.dnn_feature_columns,
                                                                                  self.embedding_dict)  # 2 x tensor:(56, 1, 8)
        sparse_embedding_input = torch.cat(sparse_embedding_list, dim=1)  # tensor: (56, 2, 8)
        senet_output = self.SE(sparse_embedding_input)  # tensor: (56, 2, 8)
        ...
        dnn_input = combined_dnn_input(temp, sparse_embedding_list)
